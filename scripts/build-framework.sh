...
if [ "$XCODE_VERSION_MAJOR" -ge 11 ]; then
  PLATFORMS+=("macCatalyst")
fi

if [ "$XCODE_VERSION_MAJOR" -ge 15 ]; then
  PLATFORMS+=("visionOS")
...
for CURRENT_PLATFORM in "${PLATFORMS[@]}"; do
  ...
  xcodebuild build -project "jmbde.xcodeproj" -destination "$DESTINATION" -scheme "jmbde" -configuration "Release" -derivedDataPath "${SRCROOT}/build/DerivedData" CONFIGURATION_BUILD_DIR="${SRCROOT}/build/${CURRENT_PLATFORM}/"
done
