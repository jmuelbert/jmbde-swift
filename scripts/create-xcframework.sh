...
then
  echo "Xcode 10 does not support xcframeworks. You can still use the individual framework for each platform."
...
do
  COMMAND_ARGS="${COMMAND_ARGS} -framework ${SRCROOT}/build/${CURRENT_PLATFORM}/jmbde.framework"
done

# Combine XCFramework
xcodebuild -create-xcframework "$COMMAND_ARGS" -output "${SRCROOT}/build/jmbde.xcframework"
open -a Finder "${SRCROOT}/build/jmbde.xcframework"
